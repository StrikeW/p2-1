 ...
 *
 *  The thread library uses the xchg instruction to lock the
 *  mutex. For threads who try to lock an already locked mutex, 
 *  they busy wait for lock to become available.
 *
 *  Since we expect Critical section to be executed in as short
 *  duration as possible, the busy wait cost should be negligible.
 *
 ...
 *    
 *  mutex datastructure initialised to default values. 
 *    
 *  @param  mp: mutex
 ...
 *
 *  Using the atomic xchg instruction to lock the mutex.
 *  If the mutex is already locked, then wait for it to become 
 *  available.
 *
 ...
 *   
 *  Unlock mutex by setting lock member variable to UNLOCK
 *
