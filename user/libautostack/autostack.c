 ...
 *         handler
 *
 *  The page fault handler for the main thread
 *  obtains the faulting address and alligns the address
 *  to a page and extends stack to the faulting address.   
 *   
 *  If a non-main thread encounters a page fault 
 *  due to stack overflow, we 
 *  terminate the thread.
 * 
 *  @param arg - arguments to the handler
 *  @param ureg - register state containing the cause for 
 *                exception
 *  @return void
 */
...
void page_fault_handler(void *arg, ureg_t *ureg)
{
    if(ureg->cause == SWEXN_CAUSE_PAGEFAULT){
        if( ! IS_MULTI_THREADING_ENABLED() ) {
            ...
            if(new_pages((void*)stack_start,cur_stack_base_addr - stack_start) < 0)
            {
                lprintf("\n new pages failed \n");
                exit(-2);
            }
 ...
 *         
 *  Get the stack high and stack low for the main
 *  thread
 *
