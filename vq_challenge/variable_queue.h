...
#ifndef __VARIABLE_QUEUE_H_
#define __VARIABLE_QUEUE_H_
 ...
 *
 *  @brief Generates a new structure of type Q_HEAD_TYPE representing the
 *  head of a queue of elements of type Q_ELEM_TYPE.
 *
 *  Usage: Q_NEW_HEAD(Q_HEAD_TYPE, Q_ELEM_TYPE); //create the type <br>
 *         Q_HEAD_TYPE headName; //instantiate a head of the given type
 *
 *  @param Q_HEAD_TYPE the type you wish the newly-generated structure to
 *         have.
 *
 ...
 *
 *  @brief Instantiates a link within a structure, allowing that structure
 *  to be collected into a queue created with Q_NEW_HEAD.
 *
 *  Usage: <br>
 ...
 *  } Q_ELEM_TYPE; <br>
 *
 *  A structure can have more than one link defined within it, as long as
 *  they have different names. This allows the structure to be placed in
 *  more than one queue simultanteously.
 *
 ...
 *
 *  @brief Initializes the head of a queue so that the queue head can be
 *  used properly.
 *
 ...
 *
 *  @brief Inserts the queue element pointed to by Q_ELEM at the front of
 *  the queue headed by the structure Q_HEAD.
 *
 *  The link identified by LINK_NAME will be used to organize the element
 *  and record its location in the queue.
 *
...
#define Q_GET_FRONT(Q_HEAD)                                 \
	((Q_HEAD)->size == 0 ? NULL : (Q_HEAD)->head)
...
#define Q_GET_TAIL(Q_HEAD)                                  \
	((Q_HEAD)->size == 0 ? NULL : (Q_HEAD)->tail)


/** @def Q_GET_NEXT(Q_ELEM, LINK_NAME)
 *
 *  @brief Returns a pointer to the next element in the queue, as linked to
 ...
 *
 *  @brief Returns a pointer to the previous element in the queue, as linked
 *  to by the link specified with LINK_NAME.
 *
...
#define Q_INSERT_AFTER(Q_HEAD,Q_INQ,Q_TOINSERT,LINK_NAME)               \
	...
	if ((Q_HEAD)->tail == (Q_INQ)) {                            \
		...
		printf("Inserting after in empty queue");                   \
	}
...
#define Q_INSERT_BEFORE(Q_HEAD,Q_INQ,Q_TOINSERT,LINK_NAME);             \
	if ((Q_HEAD)->size != 0) {                                      \
		...
		printf("Inserting before in empty queue");                   \
	}

/** @def Q_REMOVE(Q_HEAD,Q_ELEM,LINK_NAME)
 *
 *  @brief Detaches the element Q_ELEM from the queue organized by
 *  LINK_NAME, and returns a pointer to the element.
 *
 *  If Q_HEAD does not use the link named LINK_NAME to organize its elements
 *  or if Q_ELEM is not a member of Q_HEAD's queue, the behavior of this
 *  macro is undefined.
 *
 ...
 *
 *  Q_FOREACH constructs the head of a block of code that will iterate
 *  through each element in the queue headed by Q_HEAD. Each time through
 *  the loop,
